...
const RECENT_ACTIVITIES = [
  ...
  },
  {
    ...
    status: 'Completed',
    icon: 'rocket_launch',
    iconColor: 'text-orange-600 dark:text-orange-400 bg-orange-100 dark:bg-orange-900/30',
  },
];
      ...
      <aside className="w-64 bg-white dark:bg-[#111418] border-r border-slate-200 dark:border-slate-800 flex flex-col justify-between shrink-0 z-20">
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 overflow-y-auto py-6 px-3 flex flex-col gap-1">
            <Link to="/" className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-primary/10 text-primary transition-all group">
              <span className="material-symbols-outlined group-hover:scale-110 transition-transform" style={{ fontVariationSettings: "'FILL' 1" }}>grid_view</span>
              <span className="text-sm font-medium">Dashboard</span>
            </Link>
            <Link to="/schedule" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-all group">
            ...
            <button onClick={() => navigate('/settings')} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all group">
            ...
            <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800/50 px-3">
              ...
              <button onClick={onLogout} className="flex w-full items-center gap-3 px-3 py-2.5 rounded-xl text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 transition-all group text-left">
        ...
        <header className="h-16 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md sticky top-0 z-10 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8">
          ...
          <div className="flex items-center gap-4">
            <div className="relative hidden md:block">
              ...
              <input className="pl-10 pr-4 py-2 bg-slate-100 dark:bg-slate-900 border-none rounded-lg text-sm w-64 focus:ring-2 focus:ring-primary transition-all placeholder:text-slate-500" placeholder="Search meetings..." />
            </div>
          ...
          <div className="max-w-7xl mx-auto flex flex-col gap-8">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
              ...
              <div className="text-right hidden md:block">
                <p className="text-sm font-medium text-slate-500 uppercase tracking-wider">Current Time</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <button onClick={() => navigate('/waiting-room')} className="relative group overflow-hidden rounded-xl p-6 flex flex-col justify-between h-40 bg-gradient-to-br from-[#137fec] to-[#0b5cbe] text-white shadow-lg shadow-blue-500/20 transition-all hover:shadow-blue-500/40 hover:-translate-y-1">
                <div className="bg-white/20 p-2 rounded-lg backdrop-blur-sm w-fit"><span className="material-symbols-outlined text-3xl">videocam</span></div>
                <div className="text-left z-10"><h3 className="text-lg font-bold">New Meeting</h3><p className="text-blue-100 text-sm">Start an instant meeting</p></div>
              ...
              <button className="relative group overflow-hidden rounded-xl p-6 flex flex-col justify-between h-40 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm transition-all hover:border-primary/50 hover:-translate-y-1">
                <div className="bg-orange-100 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400 p-2 rounded-lg w-fit"><span className="material-symbols-outlined text-3xl">add_link</span></div>
                <div className="text-left"><h3 className="text-lg font-bold">Join Meeting</h3><p className="text-slate-500 text-sm">Via ID or personal link</p></div>
              </button>
              <button onClick={() => navigate('/schedule')} className="relative group overflow-hidden rounded-xl p-6 flex flex-col justify-between h-40 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm transition-all hover:border-primary/50 hover:-translate-y-1">
                <div className="bg-purple-100 dark:bg-purple-500/20 text-purple-600 dark:text-purple-400 p-2 rounded-lg w-fit"><span className="material-symbols-outlined text-3xl">calendar_month</span></div>
                <div className="text-left"><h3 className="text-lg font-bold">Schedule</h3><p className="text-slate-500 text-sm">Plan ahead for later</p></div>
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                      ...
                      <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                        {RECENT_ACTIVITIES.map((activity) => (
                          <tr key={activity.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-slate-500">ID: {activity.meetingId}</p>
                                </div>
                              </div>
                            </td>
                            ...
                            <td className="px-6 py-4">
                              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${activity.status === 'Completed' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-slate-100 text-slate-800 dark:bg-slate-700 dark:text-slate-400'}`}>
                                {activity.status}
                              </span>
                            </td>
                            <td className="px-6 py-4 text-right">
                              <button className={`text-sm font-medium px-3 py-1.5 rounded-lg transition-colors ${activity.status === 'Completed' ? 'bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600' : 'bg-primary/10 text-primary hover:bg-primary hover:text-white'}`}>
                                {activity.status === 'Completed' ? 'View Recording' : 'Rejoin'}
                              </button>
                            </td>
                          </tr>
                        ))}
              ...
              <div className="xl:col-span-1 flex flex-col gap-4">
                ...
                <div className="bg-[#1c2127] dark:bg-slate-800 rounded-xl border border-slate-700/50 shadow-lg relative overflow-hidden flex flex-col h-full min-h-[300px]">
                  <div className="h-32 bg-cover bg-center relative" style={{ backgroundImage: "url('https://picsum.photos/400/200?random=5')" }}>
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    <div className="flex-1">
                      <h4 className="text-2xl font-bold text-white mb-1">{UPCOMING_MEETING.title}</h4>
                      <p className="text-slate-400 text-sm mb-4">{UPCOMING_MEETING.team}</p>
                      ...
                      <div className="flex -space-x-3 mb-6">
                        {UPCOMING_MEETING.participants.map((avatar, idx) => (
                          <div key={idx} className="size-8 rounded-full border-2 border-[#1c2127] bg-cover bg-center" style={{ backgroundImage: `url(${avatar})` }}></div>
                        ))}
                        <div className="size-8 rounded-full border-2 border-[#1c2127] bg-slate-600 flex items-center justify-center text-xs font-medium text-white">
                          +{UPCOMING_MEETING.totalParticipants - UPCOMING_MEETING.participants.length}
                        </div>
                      </div>
                    </div>
                    <button onClick={() => navigate('/waiting-room')} className="w-full bg-primary hover:bg-blue-600 text-white font-bold py-3 px-4 rounded-xl transition-all shadow-lg flex items-center justify-center gap-2">
